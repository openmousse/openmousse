...
from mousse_common import api  # noqa: E402


def agents() -> dict[str, str] | None:
    """现在有哪些 Agent；服务连不上返回 None。"""
    try:
        return {g["id"]: f"{g['name']}：{g.get('purpose') or ''}" for g in api("/api/groups", timeout=10)["groups"]}
...
def main() -> None:
    ...
    if a.list or not a.agent or not a.question:
        if known is None:
            print("服务没连上（~/.openmousse/server.json 的 bind 地址，systemctl --user status openmousse-server）")
        elif not known:
            print("还没有 Agent。用户可以在 app 里新建，或者你用 agent-builder skill 建。")
        for k, v in (known or {}).items():
            print(f"{k}\t{v}")
        return
    if known and a.agent not in known:
        print(f"没有叫 {a.agent} 的 Agent。现在有：{', '.join(known) or '（无）'}")
        sys.exit(1)
    try:
        ...
        print(f"转不过去（HTTP {exc.code}）：{'这个 Agent 上一条还没回完，等一会再问' if exc.code == 409 else detail}")
        sys.exit(1)
    except (urllib.error.URLError, TimeoutError) as exc:
        print(f"转不过去：{exc}")
        sys.exit(1)
    if r.get("status") == "timeout":
        print(f"{a.agent} 还在答（{r.get('seconds')} 秒了）。已答的部分：\n{r.get('text') or '（还没有）'}\n完整回复稍后在它的线程里。")
        sys.exit(2)
    if not r.get("ok"):
        print(f"{a.agent} 没答成：{r.get('error')}")
        sys.exit(1)
    print(r["text"])
