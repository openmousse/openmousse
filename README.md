...
**A personal-agent app and base feature set for people who run their own claw.** Self-hosted: it connects to your own OpenClaw, your own model subscriptions, your own data.

Meta's Muse deploys an agent on its servers and wraps it in an app. OpenMousse does the same wrapping, but the agent runs on your machine, the models are your accounts, and nothing leaves home.
...
**1. Install the author's TestFlight build** (fastest): after install, the connection page asks for your server address and token. The assistant's name inside the app comes from your server (`app_name` in `server.json`); Agents you create yourself; data is your own. The home-screen icon and name were fixed at build time; to change those, use option 2.
